library(TMB)
compile("gearcalibparametric.cpp",CXX="clang++")
if(is.loaded("EvalDoubleFunObject")) dyn.unload("gearcalibparametric.so")
dyn.load("gearcalibparametric.so")
...
fitmodel <- function(Species,GearNames,d)
    {
        ...
        obj <- MakeADFun(
            ...
            random=c("logspectrum","residual"),
            tracepar = TRUE,
            DLL="gearcalibparametric"
        )
        ...
        return(list(Species=Species,GearNames=GearNames,d=d,res=res,rep=rep,opt=opt,obj=obj))
    }
